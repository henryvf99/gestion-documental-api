...
import { EMITIDOS_REPOSITORY, TIPODOCUMENTO_REPOSITORY, ANIO_REPOSITORY, MES_REPOSITORY } from "@container/container";
import { EmitidosRepository, TipodocumentoRepository, AnioRepository, MesRepository } from "@core/domain/repositories";

@Injectable()
export class GetEmitidosAnioMesUseCase implements UseCase<any, any, any, any[]>{
  constructor(
  ...
  async execute(tipodocumento: string, anio: string, mes: string): Promise<any[]> {
    ...
    return this.emitidosRepository.getPorAnioMesDoc(tipodocumento, anio, mes);
  }
}